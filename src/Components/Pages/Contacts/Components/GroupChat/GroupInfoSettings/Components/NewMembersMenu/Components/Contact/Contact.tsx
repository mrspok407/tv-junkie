...
const Contact: React.FC<Props> = ({ contact, selectedMembers, handleNewMembers }) => {
  ...
  const chatMembersStatusKeys = chatMembersStatus[activeChat.chatKey].map((member) => member.key) || []
  const formatedDate = useTimestampFormater({ timeStamp: contact.lastSeen! })

  return (
    <div
      className={classNames("contact-item", {
        "contact-item--selected": chatMembersStatusKeys.includes(contact.key),
        "contact-item--new-member": selectedMembers.map((item) => item.key).includes(contact.key)
      })}
      key={contact.key}
      onClick={() => {
        if (chatMembersStatusKeys.includes(contact.key)) return
      ...
      <div className="contact-item__info">
        ...
        <div
          className={classNames("contact-item__status", {
            "contact-item__status--online": contact.isOnline || chatMembersStatusKeys.includes(contact.key)
          })}
        >
          {chatMembersStatusKeys.includes(contact.key)
            ? "Allready a member"
            : contact.isOnline
            ? "Online"
            : formatedDate
