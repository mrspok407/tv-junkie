...
import { ShowFullDataStoreState } from '../@Types'
...
const updateUserEpisodesFromDatabase = async ({ firebase }: Arguments) => {
  ...
  await Promise.all(
    Object.keys(showsLastUpdateList).map((key: any) => updateAllEpisodesWatched({ firebase, authUser, key })),
  )
  ...
  const mergedShowsEpisodes: ShowFullDataStoreState[] = mergeWith(showsFromDatabase, userShows, mergeCustomizer)

  let userShowsPromises: any = []

  mergedShowsEpisodes.forEach((show) => {
    const seasons = show.episodes.reduce((acc: any, season: any) => {
      const episodes = season.episodes.reduce((acc: any, episode: any) => {
        acc.push({
